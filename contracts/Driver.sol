...
contract Driver {
  ...
  function onlynew(address newadd) public{
  ...
  function add_Member(address _req_member,uint __aadhaar) public check_eligibility_of_payments(msg.sender) {
    ...
    emit SomeoneTriedToAddSomeone(msg.sender,_req_member);

  }

  //show aadhar refrences of a member
  function list_refrences(address _master_address) public constant returns (uint,uint,uint,uint) {
  ...
  function getPoolMoney() public constant returns (uint){
  ...
  function pool(uint __amount) public payable {
...
//Total distributable money from the pool
  function assign_loan_amount_from_pool() public constant returns (uint){
  ...
  function check_time(address ad1) public constant returns(uint)
  {
